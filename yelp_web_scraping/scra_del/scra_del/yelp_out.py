...
class YelpCrawler:
    ...
    def crawl_page(self):
        ...
        try:
            yelp_titles = self.browser.find_elements(By.CSS_SELECTOR,
                                                     'h3.css-1agk4wl')
        except:
            pass
        ...
        try:
            df.to_csv("data_out/{}_{}.csv".format(self.CITY_NAME, self.STATE_NAME), mode='a', header=False, index=False)
        except Exception as e:
            print("Exception", e)
        time.sleep(2)


city_msa = pd.read_csv('city_MSA.csv')
print(city_msa)



for i in list(range(52,len(city_msa))):
    CITY_NAME = city_msa.iloc[i]['CITY']
    STATE_NAME = city_msa.iloc[i]['State']
    BUSINESS_TYPE = 'Outdoor+Seating+Restaurants'
    RESULTS_PER_PAGE = 10
